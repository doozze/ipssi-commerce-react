...
--
-- Hôte : 127.0.0.1
-- Généré le : dim. 21 nov. 2021 à 14:57
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `connexionClient` (IN `v_mail` VARCHAR(255), IN `v_password` VARCHAR(255))  SELECT * from client
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `selectProduits` ()  SELECT * from produit$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `selectPromoCodes` ()  SELECT * from codepromo where pourcentageReduction !=0$$
...
(3, 'airPods', 149, 'célèbres écouteurs d\'Apple', 'url photo', 0, 3, 51);
